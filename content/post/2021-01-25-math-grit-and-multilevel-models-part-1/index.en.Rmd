...
Teachers tell their students that perseverance pays off in the end. However, twin studies have shown that a large chunk of learning outcomes are explained by genetics. So does sticking to it really improve grades? And if so, by how much, compared to for example IQ? And even if there were an effect of grit on grades, can it be instilled in students in school, or is it ingrained because of genetics or child rearing?

A recent study carried out in Turkey claims to show that grit does indeed matter. The study was designed in such a way that grit could shine. Effects on grades are in general bigger for younger children, while grit interventions benefit kids from lower social economic backgrounds more. The study was therefore carried out with 10 year olds from poorer families. We use data from the second wave of the study, because it allows us to make causal inferences.

We load r packages together with the data, mutate categorical variables, and have a quick look.  
...
is.ordered(data2$successr2)
```

### Visual exploration

For the experiment 16 schools were randomly divided into a control and intervention group, where special training in grit was given to the students. Let's see how schools, classes and students were divided over de two groups. 
...
```
Whereas schools were divided evenly over the groups, classes and students were not. This confirms that the researchers exerted control over the allocations of schools, but not classes and students. 

Regardless, properties such as IQ and verbal scores are pretty evenly distributed among the two groups. This is not true for wealth though, a variable that measures the income group of the parents.
...
```
The graph can be interpreted as follows. Suppose we were given a student's score on the first math test and would have to predict the score on the second math test. In that case we would predict a larger increase if we were also told that the student was in the intervention group. We would do so confidently if the student had a low score on the first test and less so if the initial score was higher. This makes sense. Fostering grit is likely to benefit those who need to work the hardest for a good score.   
...
data2 %>% 
  select(mathscore2, grit, mathscore3) %>%  # replace to your needs
  group_by(grit) %>% 
