...
Because decision trees do most of the work in the analysis, we say a little more about them. 

In a basic decision tree, the main idea is to split the data points up in sections, using all the predictors. So for the first split you make a cut at those who have free lunch, are male, scores lower than 20 points on a reading test, and so on. You hope that drop outs and graduates (the labels you predict) are already nicely separated into the two spaces (or boxes) that you made. But they probably aren't yet. So in each box you try to find new lines to split the data points on. And so on.

How does the algorithm choose the point at which to make a cut? Unless variables are continuous, it simple tries all the options. It then looks at a measure of the 'purity' inside the boxes it has made. You want data points to neatly fall in either box, so each box is pure. This can be measured by [impurity functions](https://youtu.be/a3ioGSwfVpE?list=PLl8OlHZGYOQ7bkVbuRthEsaLr7bONzbXS) such as gini or entropy. The engines in this post happen to use gini, which you may know from measurements of inequality. In the case of two classes, it multiplies the proportion of each in a box together. The resulting value has a maximum for proportions of 0.5. This is indeed the worse you can do if your goal is to make boxes pure! 

If you put no limitations and this algorithm, it will keep drawing boxes around graduates and drop out, until there is a box around every outcome measure. This is unfortunate, because we have molded our model too much to the training data. We have overfit. Hence there is bias. If we applied this model to the test data set, we would probably get very poor predictions.
