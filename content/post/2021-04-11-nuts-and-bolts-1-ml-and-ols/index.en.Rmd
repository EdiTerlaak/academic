...
Finding the maximum likelihood means maximizing the probability (or density) of values of the parameters given the data. We do this for each data point and multiply the values together (we can do this because we assume independence; we can still do likelihood inference without this assumption, but in that case things get a whole lot more complicated). 

$$\mathcal{L} (\mu_i,\sigma_{i}|y) = \prod_{i=1}^n f(y_{i}|\mu_i,\sigma_{i}) $$
We are looking then for the $\mu$ and $\sigma$ that make the data the most likely. 

Next, we take the log of both sides, because it allows us to write the product as a sum. This makes it easier to differentiate later.
...
Taking the natural log of e simplifies things considerably. Since we want to get at the maximum likelihood estimate for the mean, we drop terms that do not depend on $\mu$. 

$$ln(\mathcal{L}(\mu_i,\sigma_{i}|y))\approx \sum_{i=1}^{n}{-\frac{(y_{i}-\mu_{i})^2}{2\sigma^2}}$$
...
This is utterly unsurprising. If we assume data are normally distributed, then the mean of the data is the best candidate for the $\mu$ in the normal probability distribution formula. In other words, the normal distribution with the sample mean at its center on average assigns the highest probabilities to the data. We are not concerned with showing the power of likelihood estimation here however. We will do that later. 
...
If you multiply the Hessian by -1 you get a matrix that is called the Fisher Information (I). You do this because the second derivative at a maximum is negative. So this operation cancels that minus sign and makes the Fisher Information positive. 
...
```
We note that the estimates of the error are practically the same. The standard error of `r formatC(se[1], digits=3)` around the intercept means that we are not surprised we were off by `r formatC(p$par[1]-3, digits=3)`.
