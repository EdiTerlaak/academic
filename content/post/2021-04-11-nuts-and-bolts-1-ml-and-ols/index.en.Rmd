...
### Taking Stock
In this post we went through some of the basics of maximum likelihood in the case of linear relations and Gaussian errors. The theory only shines though when we loosen both restrictions. That will be the topic of a future post in this series on Generalized Linear Models. We can already note pros and cons though to finish off the present discussion.  
...
A nice feature of likelihood theory is that it can flexibly generate point estimates for the parameters we happen to be interested in. As we shown, simulating the most likely points from the model - with fundamental and stochastic uncertainty - does add a layer to the analysis, but actually makes the results easier to interpret.      
